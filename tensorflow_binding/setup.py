...
if "TENSORFLOW_SRC_PATH" not in os.environ:
  print(
      "Please define the TENSORFLOW_SRC_PATH environment variable.\n"
...
if not os.path.exists(os.path.join(warp_ctc_path, "libwarpctc" + lib_ext)):
  print(
      ("Could not find libwarpctc.so in {}.\n"
...
if os.getenv("TF_CXX11_ABI") is not None:
  ...
  warnings.warn(
      "Assuming tensorflow was compiled without C++11 ABI. "
...
if (enable_gpu):
  ...
  if not os.path.exists(
      cuda_inc_path) or os.readlink(cuda_inc_path) != os.environ["CUDA_HOME"]:
...
class build_tf_ext(orig_build_ext):
  ...
  def build_extensions(self):
    ...
    self.compiler.compiler_so.remove('-fno-plt')
    self.compiler.compiler_so.remove('-fno-plt')
    self.compiler.compiler_so.remove('-fno-plt')
